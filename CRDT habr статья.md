...
После мержа каждый массив next будет отсортирован всегда в одинаковом порядке. Это гарантирует нам что мы всегда знаем в какую ветку спустится сначала. Получение текста из структуры можно представить как обычный пребор графа в глубину с тем дополнением что мы знаем в какую ветку надо спустится сначала.  В примере на картинке было написано "привет" а после 3 человека написали "мир", "Вася" и "пока". Далее еще кто то написал после "мир" "!" и после "пока" "река". Поскольку в букве т слова привет у нас 3 элемента в next мы проходим этот граф в порядке определенном при мерже. Получается мы получим "привет мир ! Вася пока река"     
