...
Если с распределёнными счётчиками всё относительно просто, то с текстом и массивами возникают новые сложности. В отличие от чисел, где достаточно просто складывать значения, в тексте важен порядок символов и операций. Например, если два пользователя одновременно вставляют разные символы в одну и ту же позицию или один удаляет символ, который другой в этот момент меняет, простое объединение изменений приведёт к конфликтам и потере данных. Индексы быстро становятся ненадёжными: если кто-то вставил или удалил символ до вашей позиции, ваш индекс уже не соответствует ожидаемому месту. Поэтому для совместного редактирования текста нужны структуры, которые могут однозначно разрешать такие конфликты и сохранять корректный порядок элементов без центрального сервера.
Проблемы индексов и вставок в тексте
	•	Индексы быстро устаревают при параллельных изменениях.
	•	Операции вставки и удаления могут конфликтовать, если выполняются одновременно в одной позиции.
	•	Требуется механизм, который позволит всем участникам прийти к единому порядку символов, даже если изменения поступают в разном порядке
RGA: Replicated Growable Array
Что такое RGA и как он работает
Одним из классических решений для совместного редактирования текста является структура RGA (Replicated Growable Array). В RGA каждый элемент (например, символ текста) получает уникальный идентификатор, обычно на основе времени и идентификатора пользователя. Вставка нового элемента происходит не по индексу, а после определённого существующего элемента (по его уникальному id). Удаление реализуется как “логическое удаление”: элемент помечается как удалённый, но физически остаётся в структуре (т.н. “tombstone”), чтобы другие участники могли корректно обработать историю изменений.
Как RGA решает проблему одновременных изменений
	•	Каждый символ имеет уникальный id, поэтому даже если два пользователя вставят символы в одну и ту же позицию, их элементы не будут конфликтовать — они просто будут иметь разные id и порядок между ними можно однозначно вычислить.
	•	При удалении символа он помечается как удалённый, но не исчезает из структуры сразу, что позволяет корректно обработать все параллельные операции.
	•	Итоговый порядок символов определяется на основе уникальных id и связей между элементами (например, “вставить после id X”).
