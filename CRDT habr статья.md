...
В массивах важен не только набор элементов, но и их порядок. Индексы быстро становятся ненадёжными. Если кто-то вставил или удалил символ до вашей позиции — ваш индекс уже не совпадает с ожидаемым местом. Одним из классических решений является RGA (Replicated Growable Array). В этой структуре:
...
•	Удаление — это логическая пометка, а не физическое удаление
Вот пример	
```js
class RGAElement {
