...
Yjs — это не просто CRDT, а высокооптимизированная реализация, основанная на алгоритме YATA (Yjs Automerge-like Text Algorithm), описанном в статье 2016 года, с рядом важных улучшений производительности.[1, 2] Это делает его одним из самых популярных и эффективных решений для коллаборативных приложений сегодня.[3]

**Основные структуры данных Yjs (Shared Types):**

Yjs абстрагирует низкоуровневую логику CRDT через концепцию "Shared Types". Это привычные структуры данных JavaScript, такие как `Map` или `Array`, но с "суперспособностями": любые изменения, внесенные в них, автоматически распределяются между другими пирами и объединяются без конфликтов.[4, 5, 6]

- `Y.Text`: Это основной Shared Type для работы с текстом. Он поддерживает как обычный, так и форматированный (rich text) текст. Например, если два пользователя одновременно добавляют слова в одну и ту же фразу, `Y.Text` гарантирует, что оба изменения будут корректно объединены, в отличие от обычной строки, где одно изменение могло бы быть перезаписано.[4, 5]
    
- `Y.Array`: Используется для упорядоченных списков элементов, где порядок имеет значение.[4, 5]
    
- `Y.Map`: Для ключ-значение пар, где при конфликте последнее записанное значение для ключа "побеждает" (Last Write Wins).[4, 5]
    
- Также существуют более специализированные типы, такие как `Y.XmlElement`, `Y.XmlFragment` и `Y.XmlText`, которые обычно используются для создания сложных rich-text редакторов (например, на базе ProseMirror или Quill), но благодаря готовым интеграциям Yjs, разработчикам редко приходится работать с ними напрямую.[4]
